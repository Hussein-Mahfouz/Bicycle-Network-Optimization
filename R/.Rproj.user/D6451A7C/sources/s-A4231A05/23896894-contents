...
write_csv(msoa_distances, path = "../data/distances.csv")




## testing
testt <- flows_london[1:100,]
testt$cent_orig <- st_centroid(testt$geom_orig)
testt$cent_dest <- st_centroid(testt$geom_dest)

testt <- testt %>% 
  mutate(cent_orig = st_centroid(testt$geom_orig),
         cent_dest = st_centroid(testt$geom_dest))

dist_matrix_test <- testt %>% 
  mutate(dist =st_distance(geom_orig, geom_dest, by_element = T))

dist_matrix_test2 <- testt %>% 
  mutate(dist =st_distance(st_centroid(geom_orig), st_centroid(geom_dest), by_element = T))

dist_matrix_test3 <- testt %>% 
  mutate(dist =st_distance(orig_centroid, dest_centroid, by_element = T))
